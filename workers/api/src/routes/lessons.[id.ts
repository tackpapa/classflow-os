...
app.put('/', async (c) => {
  try {
    ...
    // TODO: 기존 app/api/lessons/:id/route.ts 로직 이식
    // 현재는 기본 응답만 반환

    return c.json({
      message: 'PUT /api/lessons/:id - Implementation needed',
      // TODO: 실제 데이터 반환
    })
  } catch (error: any) {
    console.error('[lessons.[id] PUT error:', error)
    return c.json({ error: error.message }, 500)
  }
})
...
app.delete('/', async (c) => {
  try {
    ...
    return c.json({
      message: 'DELETE /api/lessons/:id - Implementation needed',
      // TODO: 실제 데이터 반환
    })
  } catch (error: any) {
