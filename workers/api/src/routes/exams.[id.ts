...
app.get('/', async (c) => {
  try {
    ...
    if (examError || !exam) {
      return c.json({ error: '시험을 찾을 수 없습니다' }, 404)
    }
...
app.put('/', async (c) => {
  try {
    ...
    // TODO: 기존 app/api/exams/:id/route.ts 로직 이식
    // 현재는 기본 응답만 반환

    return c.json({
      message: 'PUT /api/exams/:id - Implementation needed',
      // TODO: 실제 데이터 반환
    })
  } catch (error: any) {
...
app.delete('/', async (c) => {
  try {
    ...
    return c.json({
      message: 'DELETE /api/exams/:id - Implementation needed',
      // TODO: 실제 데이터 반환
    })
  } catch (error: any) {
